...
import { Clock, User, MoreVertical, MessageCircle, AlertCircle } from 'lucide-react';
...
export function KanbanTicketCard({ ticket, settings, selectedMechanic, isDragged, onDragStart, onEdit, onShowHistory, onShowCRM }: KanbanTicketCardProps) {
  const daysInShop = differenceInDays(new Date(), parseISO(ticket.entry_date));
      ...
      <div className="flex justify-between items-start">
        ...
        <div className="flex flex-col min-w-0">
          <button 
            ...
            className="flex items-center bg-white border-2 border-zinc-800 rounded px-2 py-0.5 shadow-sm mb-1.5 w-max hover:bg-zinc-100 transition-colors"
            title="Ver historial del vehículo"
          >
            <span className="text-xs font-mono font-black text-zinc-900 tracking-widest uppercase truncate break-all">
              {ticket.id}
            </span>
          </button>
      ...
      <div className="flex items-center justify-between mt-auto">
        <div className="flex flex-col gap-1.5">
          ...
          <div className="flex items-center gap-1.5 text-[10px] text-zinc-500 font-medium">
            ...
            <span title={formatDistanceToNow(parseISO(ticket.entry_date), { addSuffix: true, locale: es })}>
              {formatDistanceToNow(parseISO(ticket.entry_date), { locale: es })}
            </span>
          </div>
        </div>
